...
void draw_cube()
{
    ...
    glVertexPointer(3, GL_FLOAT, sizeof(vertex_t), 
                    (void*)(reinterpret_cast<float*>(const_cast<vertex_t*>(cube_vertices)) + 0));
    glTexCoordPointer(2, GL_FLOAT, sizeof(vertex_t), 
                      (void*)(reinterpret_cast<float*>(const_cast<vertex_t*>(cube_vertices)) + 3));
    glNormalPointer(GL_FLOAT, sizeof(vertex_t), 
                    (void*)(reinterpret_cast<float*>(const_cast<vertex_t*>(cube_vertices)) + 5));
    glColorPointer(4, GL_UNSIGNED_BYTE, sizeof(vertex_t), 
                   (void*)(reinterpret_cast<float*>(const_cast<vertex_t*>(cube_vertices)) + 8));
...
void render_arena()
{
    rdpq_debug_log_msg("Arena");
    glPushMatrix();
    glTranslatef(-3.0f,-1.f,0);
